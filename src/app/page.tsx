...
import {
  NotebookPen, Braces, Brain, GitCompare, Database, Workflow,
  ArrowRight, Sparkles, KanbanSquare, GripVertical, CheckCircle2,
  Zap, Lock, Wifi, ChevronRight, Globe, Server, Users, GitFork,
  Map, GitBranch, HardDrive, Shield, ExternalLink, Circle,
} from "lucide-react";
...
└─────────────────────┘  └────────────────────┘
  ...
  {
    id: "mermaid", icon: Workflow, label: "Mermaid Studio", color: "#6366f1",
    href: "/app/mermaid",
...
const ROADMAP = [
  { status: "done",    label: "Rich-text notes editor (TipTap v3)",    desc: "Full slash-command support, smart paste, autosave" },
  { status: "done",    label: "DAX Insight with cited sources",         desc: "Keyword explanations with dax.guide, SQLBI & MS Learn" },
  { status: "done",    label: "Mermaid Studio (zoom, pan, export)",     desc: "Split-screen live editor with 5 themes and PNG/SVG export" },
  { status: "done",    label: "DAX Studio enterprise grid",             desc: "Monaco editor, connection profiles, 1 000-row pagination" },
  { status: "done",    label: "Kanban board (drag-and-drop)",           desc: "Task modal with 15+ fields, insights dashboard" },
  { status: "done",    label: "JSON Toolkit (tree view + validate)",    desc: "Beautify, minify, validate with tree explorer" },
  { status: "done",    label: "Text Compare (word-level diff)",         desc: "Side-by-side and inline modes, word highlighting" },
  { status: "active",  label: "Notes sidebar drag-to-reorder",         desc: "Reorder and move notes between folders" },
  { status: "active",  label: "Kanban CSV / Excel export",             desc: "Export tasks with filters to CSV and Excel" },
  { status: "planned", label: "Activity history & task timeline",       desc: "Full audit trail with timestamps on every change" },
  { status: "planned", label: "AI DAX assistant (plug-in your key)",    desc: "Natural language Q&A — cite your own sources first" },
  { status: "planned", label: "Dashboard with resizable widgets",       desc: "Drag, resize and save your own analytics layout" },
];
...
function SortableTool({ tool }: { tool: (typeof TOOLS)[0] }) {
  ...
  return (
    <div ref={setNodeRef} style={style}>
      <TiltCard className={`flex h-full select-none flex-col gap-3 p-4 ${isDragging ? "scale-105 shadow-2xl ring-2 ring-accent/50" : ""}`}>
        ...
        <div>
          ...
          <p className="mt-0.5 text-[11px] leading-relaxed text-fg-muted">{tool.tagline}</p>
        </div>
        <Link
          href={tool.href}
  ...
  return (
    <div className="relative min-h-screen overflow-x-hidden bg-bg">
      ...
      <header className="relative z-20 mx-auto flex max-w-7xl items-center justify-between px-5 py-4 sm:px-8">
        <Link href="/" className="flex items-center gap-2.5 text-[18px] font-semibold sm:text-[22px]">
          <Image src="/logo-icon.png" alt="Quick Dev Tools" width={32} height={32} priority className="h-7 w-7 sm:h-8 sm:w-8" />
      ...
      <div className="flex flex-col gap-28 pb-28">
        ...
        <section className="relative z-10 mx-auto w-full max-w-7xl px-5 pt-16 text-center sm:pt-24">
          ...
          <motion.div
            ...
            <motion.div
              ...
              {["Open Source", "Free Forever", "Local-First", "Self-Hostable"].map((tag) => (
                <span key={tag}
          ...
          <div className="relative z-10 mx-auto mt-14 grid max-w-xl grid-cols-2 gap-3 sm:max-w-2xl sm:grid-cols-4">
            {[
              ...
              <motion.div key={m.label}
                ...
                <div className="text-xl font-bold text-accent sm:text-2xl">{m.value}</div>
        ...
        <section className="relative z-10 mx-auto w-full max-w-6xl px-5">
          <motion.div
            ...
            <h2 className="text-3xl font-bold sm:text-4xl">Built different. By design.</h2>
          ...
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {[
              { icon: Globe,  title: "Open Source",     color: "#6366f1", body: "Every line on GitHub. Fork it, extend it, submit a PR. No black boxes, no hidden logic." },
              { icon: Lock,   title: "Privacy First",   color: "#10b981", body: "Notes, queries and diagrams stay in a local SQLite file. Nothing is uploaded anywhere, ever." },
              { icon: Server, title: "Self-Hostable",   color: "#f59e0b", body: "Run it on your laptop, home server, or company VM. One command — no Docker required." },
              { icon: Wifi,   title: "Works Offline",   color: "#8b5cf6", body: "All 7 tools work without internet. No API calls to external services. No spinners." },
              { icon: Zap,    title: "Genuinely Fast",  color: "#ef4444", body: "Local SQLite means sub-millisecond queries. No CDN round-trips. No spinners on every click." },
              { icon: Users,  title: "Community Driven",color: "#f97316", body: "Roadmap is public. Issues are open. Features are driven by what developers actually need." },
            ].map((c, i) => (
              <motion.div key={c.title}
                initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }} transition={{ delay: i * 0.06 }}>
                <TiltCard className="h-full">
                  <span className="mb-4 inline-grid h-11 w-11 place-items-center rounded-xl"
          ...
          <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
            <SortableContext items={tools} strategy={rectSortingStrategy}>
              <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-7">
                {orderedTools.map((tool) => <SortableTool key={tool.id} tool={tool} />)}
              </div>
            </SortableContext>
          </DndContext>
        </section>
        ...
        <section className="relative z-10 mx-auto w-full max-w-7xl px-5">
          <motion.div
            ...
            <p className="mt-3 text-fg-muted">Click any tool to explore what it does</p>
          </motion.div>
            ...
            {TOOLS.filter((t) => t.id === activeTab).map((tool) => (
              <motion.div key={tool.id}
                ...
                <div className="flex flex-col gap-5">
                  ...
                  <h3 className="text-2xl font-bold leading-snug sm:text-3xl">{tool.tagline}</h3>
                ...
                <TiltCard className="overflow-hidden p-0">
                  <div
                    ...
                    <span className="ml-3 font-mono text-[11px] text-fg-subtle">{tool.label}</span>
                  </div>
        ...
        <section className="relative z-10 mx-auto w-full max-w-5xl px-5">
          <motion.div
            ...
            <h2 className="text-3xl font-bold sm:text-4xl">Up and running in 30 seconds</h2>
            <p className="mt-3 text-fg-muted">No Docker. No cloud account. No credit card.</p>
          </motion.div>

          <div className="grid gap-5 sm:grid-cols-3">
            {[
              { step: "01", title: "Clone the repo",
                code: "git clone github.com/\nkumaresh-rgb/quickdevtool",
                note: "Public MIT-licensed repo" },
              { step: "02", title: "Start both servers",
                code: "npm install && npm run dev\n# .NET backend auto-starts",
                note: "Single command launch" },
              { step: "03", title: "Open localhost:3000",
              ...
              <motion.div key={s.step}
                ...
                <TiltCard className="h-full">
                  <div className="mb-4 font-mono text-4xl font-bold text-accent/20">{s.step}</div>
          ...
          <motion.div
            ...
            <TiltCard className="p-8 sm:p-10">
              <div className="mb-8 text-center">
                ...
                <h2 className="text-2xl font-bold sm:text-3xl">Your data stays yours. Always.</h2>
                <p className="mt-2 text-fg-muted">No account required. No telemetry. No cloud sync. Ever.</p>
              </div>
              <div className="grid gap-8 md:grid-cols-3">
                {[
                  { icon: Lock,   title: "Zero cloud",        body: "SQLite on your machine. Notes, queries and diagrams never leave your computer." },
                  ...
                  { icon: Shield, title: "Your data, always", body: "No account, no sync, no telemetry. Your work stays yours. Permanently." },
                ].map((c) => (
        ...
        <section className="relative z-10 mx-auto w-full max-w-5xl px-5">
          <motion.div
            ...
            <h2 className="text-3xl font-bold sm:text-4xl">What&apos;s coming next</h2>
              ...
              <motion.div key={item.label}
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center justify-between gap-3">
                    <p className={`text-sm font-medium ${item.status === "done" ? "text-fg-subtle line-through" : "text-fg"}`}>
                      {item.label}
                    </p>
                    <span className={`shrink-0 rounded-full px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide ${
                      ...
                      {item.status === "done" ? "Done" : item.status === "active" ? "Active" : "Planned"}
                    </span>
                  </div>
        ...
        <section className="relative z-10 mx-auto w-full max-w-3xl px-5 text-center">
          <motion.div
            ...
            <div className="mb-5 flex justify-center">
              <Image src="/logo-icon.png" alt="Quick Dev Tools" width={56} height={56} />
            </div>
            ...
            <p className="mx-auto mt-4 max-w-md text-base text-fg-muted">
              ...
              Just a workspace that works — offline, fast, forever free.
            </p>
            <p className="mt-3 text-xs text-fg-subtle">
              Crafted with ❤️ by{" "}
              <Link href="https://kumaresh.vercel.app/" target="_blank" rel="noopener noreferrer"
